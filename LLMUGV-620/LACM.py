#!/usr/bin/env python3
"""
KEEP_LAST.py  –  Baseline variant that **executes the current sub‑task ID as the
interim skill** while the asynchronous LLM (``decide``) is still running.

Key change compared with the original Keep‑Last implementation:
    * During the LLM latency, we no longer repeat the *previous* skill.
      Instead, we temporarily set ``cur_skill = cur_id`` (i.e. choose the skill
      that numerically equals the recognised sub‑task identifier).

All other functionality (reward‑10 counter, collision flag, stuck protection,
CSV logging, etc.) remains unchanged.
"""
...
# Constants
# =============================================================================
FALLBACK_SKILL = 0              # used before the first scene‐ID is available
...
def run_episode(env, scene_id: SceneID, lib: SkillLibrary) -> EpiReturn:
    ...
    # ---- state vars ----
    cur_id = prev_id = 0
    cur_skill = FALLBACK_SKILL  # use fallback until a scene‑ID is detected
    scene_id.reset()
    ...
    while not done:
        ...
        if cur_id != prev_id:
            ...
            cur_skill = cur_id       # <<< use current task ID during latency >>>
        ...
        #     "step=%d  cur_skill=%d  action=%s",
        #     + raw_steps * STEP_SCALE,  # 逻辑步编号
        #     cur_skill,
        #     action,
        #     )
        ...
        if done and reward < 0:               # collision rule
            collision_flag = 1
...
if __name__ == "__main__":
    p = argparse.ArgumentParser("Keep‑Last baseline evaluator (cur_id variant)")
    p.add_argument("--agent", required=True)
    p.add_argument("--env",   required=True)
    p.add_argument("--episodes", type=int, default=500)
    p.add_argument("--out",   type=str, default="ORACLE_results")
    args = p.parse_args()
    # python LACM.py --agent sac --env ugv --episodes 500
