...
from ugv_wrapper.base.env_wrapper import EnvWrapper
...
class UnityWrapperProcess:
    def __init__(self,
        ...
        self.engine_configuration_channel.set_configuration_parameters(
            width=1280 if train_mode else 1280,
            height=720 if train_mode else 720,
            quality_level=2,
    ...
    def step(self, ma_d_action, ma_c_action):
        ...
        if self.group_aggregation:
            for n in self.behavior_names:
                if self.ma_d_action_sizes[n]:
                    ...
                    unity_d_action = np.zeros((len(self._ma_agents_ids[n]), unity_d_action_summed_size),
                                              dtype=ma_d_action[n].dtype)

                    for action, group_id, group_id_count in zip(d_action, self._ma_u_group_ids[n],
                                                                self._ma_u_group_id_counts[n]):
                        unity_d_action[self._ma_group_ids[n] == group_id] = action.reshape(-1,
                                                                                           unity_d_action_summed_size)[
                                                                            :group_id_count]
                ...
                if self.ma_c_action_size[n]:
                    ...
                    for action, group_id, group_id_count in zip(c_action, self._ma_u_group_ids[n],
                                                                self._ma_u_group_id_counts[n]):
                        unity_c_action[self._ma_group_ids[n] == group_id] = action.reshape(-1, unity_c_action_size)[
                                                                            :group_id_count]
        ...
        for n in self.behavior_names:
            ...
            ma_obs_list[n] = [np.zeros((agents_len, *obs_shape), dtype=np.float32) for obs_shape in
                              self.ma_unity_obs_shapes[n]]
...
class UnityWrapper(EnvWrapper):
        ...
        if self._seq_envs:
            ...
            for i in range(self.env_length):
                self._envs.append(UnityWrapperProcess(conn=None,
                                                      ...
                                                      scene=scene,
                                                      additional_args=additional_args,
                                                      n_envs=min(MAX_N_ENVS_PER_PROCESS,
    ...
    def reset(self, reset_config=None):
        ...
        ma_obs_list = {n: [np.concatenate(env_obs_list)
                       ...
                       for n in self.behavior_names}
        # for n in self.behavior_names:
        #     ma_obs_list[n][0] = ma_obs_list[n][0].reshape((self.n_envs, 84, 84, 3))
        #     # ma_obs_list[n][1] = ma_obs_list[n][1].reshape((self.n_envs, 802))
        #     # ma_obs_list[n][2] = ma_obs_list[n][2].reshape((self.n_envs, 84, 84, 3))
        #     ma_obs_list[n][1] = ma_obs_list[n][1].reshape((self.n_envs, 7))
        return ma_obs_list
    ...
    def step(self,
        ...
        #     ma_obs_list[n][1] = ma_obs_list[n][1].reshape((self.n_envs, 802))
        #     # ma_obs_list[n][2] = ma_obs_list[n][2].reshape((self.n_envs, 84, 84, 3))
        #     ma_obs_list[n][2] = ma_obs_list[n][2].reshape((self.n_envs, 7))
        return ma_obs_list, ma_reward, ma_done, ma_max_step, ma_padding_mask
...
if __name__ == "__main__":
    ...
    env = UnityWrapper(train_mode=True,
                       env_name=r'F:\LLMUGV\unity\WIN\ObstacleAvoidance_2000\RLEnvironments.exe',
                       n_envs=N_ENVS,
                       group_aggregation=GROUP_AGGREGATION,
                       scene='scene_1')
    ...
    for i in range(100):
        ...
        for j in range(100):
            ...
            # print(ma_obs_list['UGVParkingAgent?team=0'][6], '----------------')
            # print(ma_done['UGVParkingAgent?team=0'][0])
            print(ma_c_action['UGVParkingAgent?team=0'][0])
